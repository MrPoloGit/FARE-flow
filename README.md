...
`make hal`, `make lge`, `make neta`, and `make deps` clone (and, for HAL/LGE, build) downloaded tools that aren't in the flake - HAL, ReGDS-Logic-Gate-Extraction, and NetA all land under `tools/` (gitignored - see `docs/flow.md` for why each lives outside Nix). `make strip-power` and `make remove-decap` are follow-up steps for decluttering a finished netlist before analysis in HAL's GUI - run in that order (`remove-decap` expects power-stripped input), and both take explicit `NET=`/`LIBERTY=` rather than assuming a specific prior target already ran, so they work on any netlist you already have too. `make clean`/`make distclean` remove generated artifacts (and, for the latter, everything under `tools/`).
